...
// item A
function nomesPets (pets){
    return pets.nome
}
    function exibeNomesPets (pets){
        const petsNomes = pets.map(nomesPets)
        return petsNomes
    }
    console.log(exibeNomesPets(pets))
           
            
// item B
function apenasRacaSalicha (predicate){
const apenasRacaSalicha = pets.filter(predicate: (valor: { nome: string; raca: string; }, index: number, array: { nome: string; raca: string; }[]) => value is { nome: string; raca: string; }, thisArg?: any): { nome: string; raca: string; }[]


    return 
}
const cahorrosSalsicha = pets.
}
console.log
            [
               { nome: "Lupin", raca: "Salsicha"},
